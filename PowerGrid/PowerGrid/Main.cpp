...
int main() {
	...
	Elektro* wallet = new Elektro();
	Elektro* wallet2 = new Elektro();

	//Part 4
	Player* player1 = new Player(1, "Trevor", wallet, "Red");
	Player* player2 = new Player(2, "Stephen", wallet2, "Blue");

	player1->getElektro()->addElektro(157);
	//player1->getElektro()->show();


	map->setPlayer(city2, player1);
	map->setPlayer(city2, player2);

	vector <RedHouse*> rh(22, new RedHouse(player1));
	...
	/*
	string filename = "C:/Users/t_lall/source/repos/PowerGrid/UsaMap.txt";
	ifstream file;
	file.open(filename);
	...
	vector<City*>city = Map::getCityVector(map);

	for (int i = 0; i < city.size();i++) {
		cout<< i << " : " << city.at(i)->getName() << endl;
	}*/
